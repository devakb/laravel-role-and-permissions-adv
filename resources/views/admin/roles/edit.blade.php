                    ...
                    <div class="col-md-6 mt-5">
                        <div style="column-count: 3;">
                            @foreach($permissions as $permission)
                                <input type="checkbox" name="permissions[]" value="{{$permission->id}}" @if($role->permissions->contains($permission->id)) checked @endif> &nbsp; <b>{{ Str::upper(str_replace('_',' ',$permission->name)) }}</b><br>
                            @endforeach
                        </div>
                        @error('permissions.*')
                            <br>
